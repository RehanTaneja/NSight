...
def make_summary_plot(model_path, data_path):
    # Load the model using onnxruntime
    session = ort.InferenceSession(model_path)
    ...
    # --- NEW: Generate a SHAP Summary Plot ---
    # The summary plot will show feature importance and impact direction
    # For multi-output models, you might need to select the first output (e.g., shap_values[0])

    if isinstance(shap_values, list):
        shap_values = shap_values[0]  # Use the first output in case of multi-output model

    # Create the SHAP Summary Plot
    shap.summary_plot(shap_values, input_data, feature_names=data.columns)
